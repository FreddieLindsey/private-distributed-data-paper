...
\cite{ivandodis:2003:inproceedings} authored improved schemes which aim to address some of the shortcomings of the original BBS scheme. Crucially, the scheme is unidirectional and non-transitive. The former is important for this project since one of the criteria is to have layered access, implying that some parties will be involved unknown to the delegator individually. Non-transitivity is equally important as it allows the publishing of re-encryption keys without risk of any malicious attacker delegating themselves rights to content for which the original delegator has not delegated access to them.
...
The paper presents two useful schemes which differ only slightly.

\begin{table}[H]
  \centering
  \begin{tabular}{ | l | l | l | }
    \hline
    Function & $2^{nd}$ iteration & $3^{rd}$ iteration \\
    \hline
    Key Generation & & \\
  ...
  \end{tabular}
  \caption{
    A comparison of the second and third iterations authored by \cite{afgh:2006:article}
  }
  \label{table:afgh_comparison}
\end{table}
